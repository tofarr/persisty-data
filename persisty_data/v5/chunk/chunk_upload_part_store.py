...
from persisty_data.v5.chunk.model.chunk_upload_part import ChunkUploadPart, ChunkUploadPartStored
...
class ChunkUploadPartStore(StoreABC[UploadPart]):
    ...
    def create(self, item: UploadPart) -> Optional[UploadPart]:
        ...
        existing_num_parts = self.chunk_upload_part_store.count(AttrFilter("upload_id", AttrFilterOp.eq, upload.id))
        chunk_upload_part = ChunkUploadPartStored(
            id=item.id,
        ...
        upload_part = self.chunk_upload_part_store.create(chunk_upload_part)
        return upload_part

    def read(self, key: str) -> Optional[UploadPart]:
        return self.chunk_upload_part_store.read(key)

    def _update(self, key: str, item: UploadPart, updates: UploadPart) -> Optional[UploadPart]:
    ...
    def search(
        ...
        return self.chunk_upload_part_store.search(search_filter, search_order, page_key, limit)
