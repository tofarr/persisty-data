...
@dataclass
class DataStoreActionFactory(ActionFactoryABC):
...
def action_for_upload_create(
    ...
    def upload_create(key: Optional[str], content_type: content_type_type, file_size: Optional[int] = None, authorization: Optional[Authorization] = None) -> BeginUploadResult:
        ...
        number_of_parts = 1
        if file_size:
            number_of_parts = math.ceil(file_size / max_part_size)
        if number_of_parts > max_number_of_parts:
            raise PersistyError('file_size_too_large')

        upload = upload_store_meta.get_create_dataclass()(
            item_key=key,
            content_type=content_type,
            max_number_of_parts=max_number_of_parts,
