...
from persisty_data.v7.routes import create_route_for_part_upload, create_route_for_download
...
class PersistyFileStoreABC(FileStoreABC, ABC):
    ...
    def get_routes(self) -> Iterator[_Route]:
        ...
        api_access = self.get_meta().store_security.get_api_access()
    ...
    def upload_create(
        ...
        edits = (
            BatchEdit(
                create_item=PersistyUploadPart(upload_id=upload_handle.id, part_number=part_number
                )
            )
